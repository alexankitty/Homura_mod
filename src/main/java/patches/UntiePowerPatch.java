...
public class UntiePowerPatch {
  ...
  public static SpireReturn<Void> Insert(
    ...
    @ByRef int[] amount,
    @ByRef float[] duration
  ) {
    if (
      target == AbstractDungeon.player &&
      AbstractDungeon.player.hasPower("UntiePower")
    ) {
      if (
        powerToApply.ID.equals("Weakened") ||
        powerToApply.ID.equals("Frail") ||
        powerToApply.ID.equals("Vulnerable")
      ) {
        AbstractDungeon.actionManager.addToTop(
          (AbstractGameAction) new TextAboveCreatureAction(
            target,
            (CardCrawlGame.languagePack.getUIString("ApplyPowerAction")).TEXT[1]
          )
        );
