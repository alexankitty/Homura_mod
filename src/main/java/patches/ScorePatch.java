...
public class ScorePatch {
  ...
  public static ScoreBonusStrings ScoreString =
    CardCrawlGame.languagePack.getScoreString("SL");
  ...
  public static class PatchVictoryScreen {
    ...
    public static SpireReturn<Void> Insert(VictoryScreen _instance) {
      ArrayList<GameOverStat> stats = (ArrayList<
          GameOverStat
        >) ReflectionHacks.getPrivate(_instance, GameOverScreen.class, "stats");
      if (
        AbstractDungeon.player.hasRelic("MetalShield") &&
        "Akemi_Homura".equals(AbstractDungeon.player.name)
      ) {
        System.out.println(ScorePatch.ScoreString.NAME);
        System.out.println(ScorePatch.ScoreString.DESCRIPTIONS[0]);
        stats.add(
          new GameOverStat(
            ScorePatch.ScoreString.NAME,
            ScorePatch.ScoreString.DESCRIPTIONS[0],
            Integer.toString(500 - Patch.countSL() * 100)
          )
        );
      }
