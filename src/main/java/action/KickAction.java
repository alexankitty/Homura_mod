    ...
    if (shouldCancelAction()) {
      ...
      if (this.isDone) {
        ...
        if (this.target.lastDamageTaken > 0) {
          ...
          addToBot((AbstractGameAction)new ApplyPowerAction(this.target, this.source, (AbstractPower)new StrengthPower(this.target, -this.target.lastDamageTaken), -this.target.lastDamageTaken));
          if (!this.target.hasPower("Artifact")) {
            addToBot((AbstractGameAction)new ApplyPowerAction(this.target, this.source, (AbstractPower)new GainStrengthPower(this.target, this.target.lastDamageTaken), this.target.lastDamageTaken, true, AbstractGameAction.AttackEffect.NONE));
          }
