  ...
  extends AbstractGameAction
{
  ...
  public void update() {
    if (this.duration == 0.1F && this.target != null) {
      int res; AbstractDungeon.effectList.add(new FlashAtkImgEffect(this.target.hb.cX, this.target.hb.cY, AbstractGameAction.AttackEffect.BLUNT_HEAVY));
      ...
      p.gainGold(res);
      for (int i = 0; i < res; i++) {
        AbstractDungeon.effectList.add(new GainPennyEffect((AbstractCreature)p, this.target.hb.cX, this.target.hb.cY, p.hb.cX, p.hb.cY, true));
      }
