...
public class Coercion extends CustomCard {
  ...
  private static final CardStrings cardStrings =
    CardCrawlGame.languagePack.getCardStrings("Coercion");
    ...
    if (m.type != AbstractMonster.EnemyType.BOSS) {
      ...
      if (HP < RadomNum) {
        ...
        if ("Darkling".equals(m.id)) {
          addToBot(
            (AbstractGameAction) new InstantKillAction((AbstractCreature) m)
          );
        } else {
          m.escape();
        }
  ...
  public void calculateCardDamage(AbstractMonster m) {
    ...
    if (
      ...
      this.baseMagicNumber = (int) Math.ceil(
        (1.0D - (m.currentHealth / m.maxHealth)) * 100.0D
      );
