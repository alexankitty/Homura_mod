...
public class Suppress extends CustomCard {
  ...
  private static final CardStrings cardStrings =
    CardCrawlGame.languagePack.getCardStrings("Suppress");
  public static final String ID = "Suppress";
  ...
  public void use(AbstractPlayer p, AbstractMonster m) {
    Iterator<AbstractMonster> monsterGroup =
      (AbstractDungeon.getCurrRoom()).monsters.monsters.iterator();
    while (monsterGroup.hasNext()) {
      AbstractMonster mo = monsterGroup.next();
      if (Patch.countCurse() >= 5) {
        addToBot(
          (AbstractGameAction) new StunMonsterAction(
            mo,
            (AbstractCreature) p,
            1
          )
      ...
      addToBot(
        (AbstractGameAction) new ApplyPowerAction(
          (AbstractCreature) mo,
          (AbstractCreature) p,
          (AbstractPower) new WeakPower((AbstractCreature) mo, 1, false),
          1,
          true,
