...
public class Stock extends CustomCard {
  ...
  private static final CardStrings cardStrings =
    CardCrawlGame.languagePack.getCardStrings("Stock");
    ...
    if (this.upgraded) {
      addToBot(
        (AbstractGameAction) new ApplyPowerAction(
          (AbstractCreature) p,
          (AbstractCreature) p,
          (AbstractPower) new StockUpPower((AbstractCreature) p, 1)
        )
      );
    } else {
      addToBot(
        (AbstractGameAction) new ApplyPowerAction(
          (AbstractCreature) p,
          (AbstractCreature) p,
          (AbstractPower) new StockPower((AbstractCreature) p, 1)
        )
      );
    }
  }
